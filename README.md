...
HAVEN is pre-release software. The current milestone adds a live, privacy-bounded Chrome profile review with explicit sorting and conservative session-signal and cleanup heuristics alongside session-theft defenses and provider-session review. It deliberately excludes cookie credentials and cannot prove whether a provider session is authenticated. Its native desktop client remains a hardened shell, its account-security notebook remains a checklist—not a provider integration or secret vault—and HAVEN is not a replacement for native protection.

## Milestone 0.20 — Chrome Profile Review
...
- A Chrome-only, per-profile site-data review that groups current cookies by domain; shows aggregate totals, session/persistent scope, Secure and HTTP-only coverage, last access, and latest persistent expiry; and supports explicit session-signal, cleanup-priority, recency, count, and domain sorting
- A searchable, profile-first review queue that calls sites unused for at least 90 days—or sites without a usable access date—review candidates and advises confirmation in Chrome or at the provider before clearing anything
...
- Chrome profile labels and grouped domains remain live-only in the latest authenticated observation, are removed from historical persistence, and never enter findings, activity history, or push notifications
...
- Stable, browser-native routes for Overview, Devices, Network, Appliances, Accounts, Activity, Settings, and per-device posture, browser, services, and history views
