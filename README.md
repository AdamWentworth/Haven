...
HAVEN is pre-release software. The current milestone adds an encrypted, owner-curated Chrome site-data workflow to its live, privacy-bounded profile review. It deliberately excludes cookie credentials, cannot prove whether a provider session is authenticated, and never clears browser data itself. Its native desktop client remains a hardened shell, its account-security notebook remains a checklist—not a provider integration or secret vault—and HAVEN is not a replacement for native protection.

## Milestone 0.21 — Browser Site Curation
