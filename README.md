...
HAVEN is pre-release software. The current milestone adds privacy-bounded session-theft defenses and provider-session review without reading the private contents that browsers protect. Its native desktop client remains a hardened shell, its account-security notebook remains a checklist—not a provider integration or secret vault—and HAVEN is not a replacement for native protection.

## Milestone 0.19 — Session Defense
...
- No collection or storage of cookies, browsing history, passwords, tokens, page contents, form data, raw extension IDs, raw site-match patterns, or profile names; browser inventory is live-only and removed from historical observations
...
- Direct shortcuts to known providers' official session/security pages; HAVEN records the owner's result but never signs in to a provider, receives an OAuth grant, enumerates sessions, or reads browser cookies
