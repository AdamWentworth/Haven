...
HAVEN is pre-release software. The current milestone adds privacy-bounded browser and extension exposure inventory without reading the private contents that browsers protect. Its native desktop client remains a hardened shell, its account-security notebook remains a checklist—not a provider integration or secret vault—and HAVEN is not a replacement for native protection.

## Milestone 0.18 — Browser & Session Protection
