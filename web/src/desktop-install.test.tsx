...
describe("desktop installation", () => {
	...
	it("recognizes the bounded native desktop user-agent marker", () => {
		userAgent.mockReturnValue("Mozilla/5.0 HAVEN-Desktop/0.22.0 Electron");
		expect(nativeDesktopVersion()).toBe("0.22.0");
		expect(isStandaloneApp()).toBe(true);
		const { result } = renderHook(() => useDesktopInstall());
		expect(result.current).toMatchObject({ status: "native", nativeVersion: "0.22.0" });
	});
