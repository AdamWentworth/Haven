...
| Chrome site-data review cannot become credential extraction | Fixed read-only aggregate query with no credential columns | Collector tests seed recognizable cookie names, plaintext values, and encrypted bytes, then require all of them to be absent from serialized output while domain/count metadata remains available; model tests enforce profile, domain, count, time, and global bounds |
...
| Cookie metadata is not overstated as a logged-in session | Domain-level evidence with a conservative age heuristic | Rendered UI tests require the distinction between cookie presence and provider authentication plus review-before-clear guidance; no provider request, logout, or cookie-clearing capability exists |
