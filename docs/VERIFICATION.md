...
| Appliance state remains bounded and credential-free | Current TCP/TLS probe result | SQLite tests persist only declared endpoint state, bounded errors, timestamps, and public certificate metadata; no request body, response body, or credential field exists |
