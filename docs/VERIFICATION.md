...
| Brand polish does not create tracking, cropping, or Windows small-icon regressions | Canonical local vector geometry plus purpose-specific generated rasters and ICO entries | Tests require a transparent standard mark with the approved gradient rim, pale keyline, dark field, and geometric H palette intact; an optically larger desktop mark; an opaque maskable tile; explicit manifest purposes; nine decodable transparent Windows sizes from 16–256px; and deterministic regeneration in CI |
...
| Routine NAS monitoring does not generate SSH-login noise when manual mode is selected | Explicit `deepCheckMode` plus the current retained health projection | Go tests prove scheduled manual-mode health calls SNMP without invoking SSH, retain prior deep evidence, run SSH exactly once after an explicit request, timestamp the result, and rate-limit immediate repeats; API and UI tests require the deliberate action and login-notification warning |
