...
HAVEN never opens browser databases or collects cookies, history, passwords, session tokens, profile names, page contents, or form data. A malicious extension can still hide behavior from manifest metadata, and an already-compromised endpoint can suppress or falsify its report. These features reduce exposure and make selected changes reviewable; they do not prove that a session was not stolen or replace malware protection and provider-side session review.
