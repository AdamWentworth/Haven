...
Milestone 0.7.5 performs continuous collection, explainable baseline evaluation, privacy-bounded finding and listener transition tracking, authenticated device reporting, owner-reviewed expected-service classification, explicit report freshness/lifecycle presentation, live Docker workload and Linux systemd-unit attribution, and an authenticated local action boundary. Suggested baseline entries are derived only from bounded platform roles and the current live observation; nothing is classified until the owner approves it. On Windows, the collector launches one fixed, internally defined PowerShell script with no user-controlled commands or parameters. The two available controls also select fixed internal PowerShell commands for a Defender quick scan or security-intelligence update; browser text is never interpolated into them.
...
The activity ledger records only finding-opened and finding-resolved transitions using the already bounded finding text. Listener history is a separate bounded baseline containing protocol, port, bind scope, and timestamps; raw addresses, remote endpoints, process history, systemd ownership history, and payloads are not persisted. An approved expected-service record may contain a small owner-selected process, workload, or systemd-unit allowlist, but this is configuration metadata rather than observation history; audit summaries deliberately omit those names. Browser desktop alerts are opt-in, contain no secrets, and operate only while the page is open. HAVEN does not register a privileged tray process or notification service in this milestone.
