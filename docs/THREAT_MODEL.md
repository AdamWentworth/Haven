...
The activity ledger records only finding-opened and finding-resolved transitions using the already bounded finding text. Listener history is a separate bounded baseline containing protocol, port, bind scope, and timestamps; raw addresses, remote endpoints, process history, and payloads are not persisted. An approved expected-service record may contain a small owner-selected process or workload allowlist, but this is configuration metadata rather than observation history; audit summaries deliberately omit those names. Browser desktop alerts are opt-in, contain no secrets, and operate only while the page is open. HAVEN does not register a privileged tray process or notification service in this milestone.
