...
The installable desktop experience does not widen the authenticated browser boundary. Its manifest, icons, shortcuts, and service worker are same-origin static assets. The service worker has no fetch handler or Cache API use, so it does not create an offline copy of authenticated pages, observations, or decrypted account notes. Installation creates no native command API, privileged helper, local database, or second credential store; the browser remains responsible for passkeys, cookies, installation, notification permission, and application-window isolation.
