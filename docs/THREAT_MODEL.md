...
HAVEN must not store passwords, cookie names or values, session tokens, authenticator seeds, recovery-code contents, or general-purpose remote-administration credentials.

## Current boundary

HAVEN performs continuous collection, explainable baseline evaluation, privacy-bounded browser and extension exposure inventory, live Chrome domain-level cookie metadata review, encrypted owner curation of observed profile/domain pairs, a deterministic browser-hardening review, endpoint-local extension-change detection, bounded Chrome session-protection evidence, finding and listener transition tracking, authenticated device reporting, owner-reviewed expected-service classification, explicit report freshness/lifecycle presentation, hub-owned fact-based active-alert derivation, opt-in encrypted Web Push delivery, live Docker workload and Linux systemd-unit attribution, a derived network-wide overview, an authenticated local action boundary, an encrypted owner-reported account-security and provider-session-review notebook, same-origin browser-managed application installation, a hardened Electron desktop client, and privacy-bounded operational diagnostics. Its canonical shield-and-H mark and purpose-specific icon variants are static local assets and make no third-party request. Product source, private deployment configuration, and private state are explicit separate recovery layers. Diagnostics expose only generic check conclusions and redacted recovery steps: they omit paths, hostnames, addresses, device identities, certificate contents, account details, and secrets. Doctor commands are read-only and do not repair, enroll, rotate trust, migrate an offline database, or contact a provider. The hardening review cannot treat cookie volume, cookie domains, inferred sign-in state, or remaining logged in as a failure; those facts never enter its deterministic projection. Higher extension access remains a one-time inventory prompt rather than a malware verdict or background alert. The overview distinguishes enrolled devices from observed-only private endpoints and grouped Internet relationships. It uses only the latest live connection and browser reports, performs no active discovery, and persists no remote endpoint, relationship, or routine browser inventory history. Suggested service-baseline entries are derived only from bounded platform roles and the current live observation; nothing is classified until the owner approves it. Alerts are review prompts based on current evidence, not intrusion verdicts or Internet-reachability claims. On Windows, the collector launches one fixed, internally defined PowerShell script with no user-controlled commands or parameters. The two available controls also select fixed internal PowerShell commands for a Defender quick scan or security-intelligence update; browser text is never interpolated into them.
...
The network overview is a browser-side projection of the latest authenticated reports. Address matches can explain likely enrolled-device relationships, but they are not cryptographic proof that the peer at an address is the enrolled machine. Unmatched private endpoints remain visibly "observed only," are never promoted to trusted inventory, and are not retained as asset history. Internet destinations are grouped to reduce noise rather than classified as benign.

The Accounts workspace is an owner-authenticated private notebook, not an account-provider integration. The owner supplies all facts manually, including whether current provider-side sessions were recognized, when they were reviewed, and which fixed session checks were completed. Static official-provider links are navigation shortcuts only. HAVEN never authenticates to those providers, receives provider OAuth grants, or enumerates their sessions; the separate Chrome site-data view cannot establish provider authentication. Each profile is encrypted as one identity-bound AES-GCM envelope with a dedicated key outside SQLite; audit history receives only the opaque profile ID and operation. The schema offers no secret or token fields, strict decoding rejects unknown fields, and recognizable TOTP setup links, private keys, and cookie-header formats are refused in free text. Free-form text cannot be perfectly classified, so the interface repeatedly instructs the owner never to paste secret contents. Losing the notebook key makes the ciphertext unreadable; therefore the complete state directory must be backed up together. Suggestions are deterministic checklist guidance, not incident findings, and are excluded from active alerts and Web Push.

Browser inventory reads only bounded product metadata and extension manifests/state belonging to the reporting user. Before reporting, raw extension IDs become one-way truncated fingerprints and raw host permissions become `none-declared`, `specific-sites`, or `all-sites`. Only an explicit capability allowlist can enter the message. For Chrome only, an owner-requested fixed SQLite query opens each cookie database read-only and selects `host_key` plus aggregate counts, boolean flags, and maximum access/expiry timestamps. It never selects cookie `name`, `value`, `encrypted_value`, or `path`. Bounded profile display labels and grouped domains are sensitive operational metadata: they travel only inside the authenticated mTLS observation, are visible only through the authenticated dashboard, and disappear from the hub when the current in-memory observation is replaced or the hub restarts. The hub independently validates all bounds and removes the complete browser object before historical persistence. If the owner deliberately classifies a currently observed domain, HAVEN stores only its device/browser identity, one-way profile fingerprint, normalized domain, and one of four fixed decisions inside a separately keyed AES-GCM envelope. Its opaque record identity is derived with a purpose-separated HMAC key, and audit history omits the domain. The endpoint keeps a mode-0600 privacy-reduced extension baseline so it can report only installations, re-enablement, site-access expansion, and added allowlisted capabilities after a first silent baseline; profile labels and domains never enter that baseline. The baseline advances only after hub acceptance. Version changes, removals, and reductions do not alert. Evaluated extension-change findings and activity entries may retain the friendly extension name and coarse change summary, but never raw identifiers or origins. Cookie metadata and classifications never create a finding, alert, activity entry containing the domain, or push notification.

On Windows, the collector reads two fixed Chrome policy values and counts at most 50 Application-log events from source `Chrome` with ID 257 during the previous seven days. It retains no event message, property, process identity, cookie identifier, or cookie value. An unset App-Bound Encryption policy means Chrome enables the protection whenever possible; it is not the same claim as an explicit policy. An unset Device Bound Session Credentials policy follows Google's rollout and does not establish that every provider or Google session is device-bound. Event 257 is evidence of a failed Chrome verification that may reflect incompatibility or an attempted bypass; it is not proof of malware.

HAVEN never copies a browser database or collects cookie names, cookie values, encrypted cookie values, cookie paths, browsing-history entries, passwords, session tokens, raw profile paths, page contents, or form data. A domain-level cookie row may represent authentication, preferences, analytics, or ordinary site operation; an access timestamp can be stale or absent, and provider-side revocation may not immediately remove local cookies. The 90-day dormant rule is therefore a review heuristic, not proof of a live login or a command to clear data. The session-signal sorter is also intentionally limited: its stronger tier requires session-scoped, Secure, and HTTP-only aggregate counts, but those attributes are common to non-authentication cookies and cannot establish current login state. The owner's `Signed in — keep` decision suppresses HAVEN cleanup suggestions only; it does not make browser storage harder to steal. Cleanup is guided back to Chrome and is never automatic. A malicious extension can still hide behavior from manifest metadata, and an already-compromised endpoint can read or falsify the same local facts. These features reduce exposure and make selected changes reviewable; they do not prove that a session was not stolen or replace malware protection and provider-side session review.
