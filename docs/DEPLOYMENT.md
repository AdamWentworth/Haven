...
Every current report carries the reporter's public release, immutable source revision, platform, bounded installation kind, observed capabilities, and current collection-notice count. The hub compares this with its own build and displays drift as maintenance only. Milestone 0.18 reporters add an optional privacy-bounded browser-security object under observation schema 2; milestone 0.19 extends that object with fixed Chrome session-protection evidence and optional meaningful extension changes. Deploy the 0.19 hub before updating agents because older strict decoders do not recognize the new protection identifiers or change field. The console-free Windows artifact embeds `windows-task` so an older scheduled task that still invokes plain `report` remains truthful after an in-place binary update; systemd invocations are recognized from systemd's own execution environment. Older reporters remain protocol-compatible and visible without 0.19 session-defense evidence until updated; no hub process pushes or installs binaries on an endpoint.
...
Deploy the 0.19 hub before updating an endpoint binary. The newer hub accepts older reports without change or Chrome session-defense evidence, while an older strict decoder rejects the new values. No database migration is required for account-session fields because each account profile is one encrypted version-tolerant document. After the hub readiness check succeeds, update reporters one endpoint at a time and require a current report before proceeding to the next device.
