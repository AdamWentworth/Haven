...
Milestone 0.20 adds a live Chrome profile site-data review while retaining the 0.19 session-defense evidence, browser inventory, hardened Electron client, installable-web fallback, private account-security notebook, verifiable fleet lifecycle evidence, routed console, reliability guardrails, and bounded read-only NAS health evidence:
...
- Routine browser inventory—including Chrome profile labels and domains—exists only in the latest in-memory observation. Historical persistence strips the browser-security object. A separate endpoint-local mode-0600 extension baseline keeps only browser identity, one-way extension fingerprint, friendly name, state, coarse site-access categories, and allowlisted sensitive capabilities; it contains no profile label, domain, cookie metadata, token, or browser-storage value.
