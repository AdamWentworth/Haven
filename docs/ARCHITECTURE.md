...
- A managed appliance is an explicit private deployment record, not an enrolled endpoint and not an automatically trusted observed asset. The hub probes only its declared TCP endpoints, performs an observation-only TLS handshake when configured, and stores only current bounded status. It never authenticates to the appliance, follows the result into discovery, or grants it endpoint-agent trust.
...
Managed appliances form a third category: explicitly configured but agentless. Their evidence is independently observed by the hub and therefore cannot establish host firewall, malware-protection, account, disk, or firmware health. Two consecutive required-service failures are necessary before an availability alert is derived. Optional visibility-only endpoints remain factual inventory and never create outage alerts.
