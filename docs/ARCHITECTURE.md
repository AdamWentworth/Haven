...
- The Linux collector asks `ss` for extended socket metadata and keeps only a sanitized terminal `.service` or `.socket` unit name. For socket-activated user services whose socket belongs to the generic user manager, it may read that same process's `/proc/<pid>/cgroup` entry; full cgroup paths, command lines, and historical process ownership are never retained.
