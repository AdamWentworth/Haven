...
- Chrome reporters read the bounded `profile.info_cache` display name used by Chrome's profile chooser, with the profile-local preference as fallback. They open each current user's cookie database read-only and execute a fixed aggregate query that selects only domain, counts, boolean security flags, and maximum access/expiry timestamps. The protocol accepts a bounded profile label, one-way profile fingerprint, and at most 256 grouped domains per profile and 1,024 per report. Cookie presence is site-data evidence, not proof that the owner is signed in.
