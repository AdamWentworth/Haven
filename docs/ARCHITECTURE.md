...
Milestone 0.19 adds session-defense evidence while retaining the 0.18 browser inventory, hardened Electron client, installable-web fallback, private account-security notebook, verifiable fleet lifecycle evidence, routed console, reliability guardrails, and bounded read-only NAS health evidence:
...
- Routine browser inventory exists only in the latest in-memory observation. Historical persistence strips the browser-security object. A separate endpoint-local mode-0600 baseline keeps only browser identity, one-way extension fingerprint, friendly name, state, coarse site-access categories, and allowlisted sensitive capabilities; it contains no version, profile count, raw ID, host pattern, cookie, token, or browser-storage value.
